...
CORS_ALLOWED_ORIGINS="${NINEBOT_CORS_ORIGINS:-$FRONTEND_ORIGIN}"

if [[ -z "${NINEBOT_CORS_ORIGINS:-}" ]]; then
  if [[ "$FRONTEND_HOST" == "127.0.0.1" ]]; then
    CORS_ALLOWED_ORIGINS="$FRONTEND_ORIGIN,http://localhost:${FRONTEND_PORT}"
  elif [[ "$FRONTEND_HOST" == "localhost" ]]; then
...
(
  cd "$BACKEND_DIR"
  env NINEBOT_CORS_ORIGINS="$CORS_ALLOWED_ORIGINS" NINEBOT_HOST="$HOST" NINEBOT_PORT="$PORT" "$VENV_PYTHON" -m app.main
) &
BACKEND_PID=$!
